...
# Keep paths anchored to this project folder so the app does not depend on,
# or expose, any personal parent directory.
PROJECT_DIR = Path(__file__).resolve().parent
DATA_FILE_DEFAULT = PROJECT_DIR / "Complete Training Dataset (All NACA Airfoils).xlsx"
...
def load_model_and_preprocessors():
    model = keras.models.load_model(str(MODELS_DIR / "aero_mlp.keras"), compile=False)
    x_scaler = joblib.load(MODELS_DIR / "x_scaler.pkl")
    y_scaler = joblib.load(MODELS_DIR / "y_scaler.pkl")
    airfoil_encoder = joblib.load(MODELS_DIR / "airfoil_encoder.pkl")
